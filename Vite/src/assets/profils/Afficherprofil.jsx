...
const Afficherprofil = ({profils,role}) => {
    ...
    fetch('http://localhost:3030/getevenementsbyemail', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('token')}` ,
  ...
  fetch('http://localhost:3030/getblogsbyemail', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('token')}` ,
        ...
        <div className="profil-preview  flex flex-col justify-center" key={profil.id}>
          ...
          <div className='flex flex-row justify-between w-[100%]'>
            ...
            <div className='flexflex-col justify-end mb-24'>
              ...
              <h2 className="text-4xl font-black p-6">
                {evenements.map((event, index) => (
                  <span className={index !== 0 ? 'ml-4 ' : ''}>
                    {event.title} {index !== evenements.length - 1 ? " ," : ""}
                  </span>
                ))}
              </h2>
              <h2 className='text-4xl font-black p-6'>
                {blogs.map((blog, index) => (
                  <span className={index !== 0 ? 'ml-4 ' : ''}>
                    {blog.title} {index !== blogs.length - 1 ? " ," : ""}
                  </span>
                ))}
              </h2>
            </div>
